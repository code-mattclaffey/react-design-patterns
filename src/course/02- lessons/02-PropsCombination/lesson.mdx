...
- image - the image sources, alt text
- cta - text and ctaUrl
...
```jsx
const CardComponent = ({ title, subText, cta, image, mobile }) => (
  <article>{/* Lots of imaginary code here... */}</article>
);
```

It is now a lot more simpler to understand, we know the component has a title, it also needs a cta and an image. If we then wanted to, we would add some styles in case we need to tweak the card.
...
[Feedback](https://forms.gle/TTDGAM24M5w1EZpf8)
